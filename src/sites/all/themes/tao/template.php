...
function tao_preprocess_html(&$vars, $hook) {
  ...
  if (!empty($vars['page']['left']) && !empty($vars['page']['right'])) {
    $vars['classes_array'][] = 'two-cols';
  ...
  // 2 COLUMNS
  elseif (!empty($vars['page']['left'])) {
    $vars['classes_array'][] = 'one-col left-only';
    
	$vars['left_classes'] = 'col-left span-8';
	$vars['center_classes'] = 'col-center span-16 last';
  
  }
  elseif (!empty($vars['page']['right'])) {
	$vars['classes_array'][] = 'one-col right-second';
